...
void network_sender_task(void* pv_param)
{
    ...
    for (;;) {
        ...
        if (net_state != MIRA_NET_STATE_JOINED) {
            log_line("Waiting for network (state is %s)",
            ...
            if (res != MIRA_SUCCESS) {
                ...
                tfp_sprintf(message, "Hello from FreeRTOS with BLE");
                mira_net_udp_send_to(conn, &net_address, UDP_PORT, message, strlen(message));
